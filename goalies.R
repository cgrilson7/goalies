...
goalies <- read_csv("https://moneypuck.com/moneypuck/playerData/seasonSummary/2022/regular/goalies.csv", col_types = cols(playerId = col_integer(), 
                                                                                                                          season = col_integer(), games_played = col_integer(), 
                                                                                                                          icetime = col_double())) %>%
...
ui <- fluidPage(
  ...
  fluidRow(width = 12, align = 'center',
           h1("NHL Goalies: Performance (GA/60) vs. Expected (xGA/60)"),
  ...
  fluidRow(align = 'center',
           ...
           h4("The vertical lines represent the average expected goals per 60 minutes for goalies shown (dark grey), and +/- one standard deviation around this average (light grey)."),
           h4("The diagonal lines represent various levels of goaltender performance vs. their expected goals against:"),
           h4(strong("Purple - worse than expected", style = "color: #440154"), " (+0.5 G-xG per 60)"),
           h4(strong("Blue - performing as expected", style = "color: #31688e")),
           h4(strong("Green - better than expected", style = "color: #35b779"), " (-0.5 G-xG per 60)")
  ),
  fluidRow(align = 'center',
           plotOutput('plot', height = "1000px", width = "80%",
                      hover = hoverOpts(
             id = 'plot_hover',
             delay = 100,
             delayType = 'throttle'
...
output$plot <- renderPlot({
  ...
  goalies_ongoal_filtered <- goalies_situation_filtered %>%
    dplyr::filter(icetime > quantile(icetime, (input$icetime_filter/100)))
  ...
  goalies_ongoal_filtered %>%
    ...
    geom_abline(slope = 1, intercept = 0.5,  color = "#440154", size = 6, alpha = 0.7) +
    geom_abline(slope = 1, intercept = 0,  color = "#31688e", size = 6, alpha = 0.7) +
    geom_abline(slope = 1, intercept = -0.5,  color = "#35b779", size = 6, alpha = 0.7) +
    geom_vline(xintercept = xg60_avg - xg60_sd, color = "grey70", size = 4, alpha = 0.5) +
    geom_vline(xintercept = xg60_avg, color = "grey60", size = 4, alpha = 0.75) +
    geom_vline(xintercept = xg60_avg + xg60_sd, color = "grey70", size = 4, alpha = 0.5) +
    geom_point(aes(size = icetime/(60*60)), color = "grey10") +
    ggrepel::geom_text_repel(aes(label = last_name), size = 6, color = "grey10", max.overlaps = 20) +
